...
local function emit()
    ...
    "playerctl metadata --format 'title_{{title}}album_{{album}}artist_{{artist}}' && playerctl status", function(out)
...
local subscribe =
[[ bash -c "playerctl metadata --format 'title_{{title}}album_{{album}}artist_{{artist}}' -F & playerctl status -F" ]]
