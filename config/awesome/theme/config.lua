...
local done = hovercursor(wibox.widget({
    ...
    buttons = {
        awful.button({}, 1, function()
            ...
            w:write(require("json.json").encode(user))
            w:close()
...
local function guiselect(name, varname, var, default, class)
    ...
    if class == "file" then
        ...
        pathcommand =
        "yad --file --add-preview --large-preview --title='select a file' --file-filter='image | *.png *.jpg *.jpeg' --width=850 --height=700"
    ...
    buttons = {
        awful.button({}, 1, function()
            awful.spawn.easy_async_with_shell(
                "~/.config/awesome/color/" .. colorentries[colorpos] .. "/" .. colorentries[colorpos] .. ".sh"
            )
...
local general = wibox.widget({
    ...
    inputbox("weather location", "location", user.location, "London"),
    visible = true,
    spacing = dpi(15),
...
local app = wibox.widget({
    ...
    inputbox("browser", "browser", user.browser, "firefox"),
    visible = false,
    spacing = dpi(15),
    layout = wibox.layout.fixed.vertical,
})
...
awesome.connect_signal("widget::config", function()
    ...
    awful.placement.centered(configdisplay, {
